...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        self.model = GRUDecoder(
            ...
            use_batch_norm = self.args['model'].get('use_batch_norm', False),
            batch_norm_momentum = self.args['model'].get('batch_norm_momentum', 0.1),
        )
        ...
        # Get the exclude_val_blocks_dict from config (for filtering out OpenWebText/Random corpus from validation)
        exclude_val_blocks_dict = self.args['dataset'].get('exclude_val_blocks_dict', None)
        ...
        train_trials, _ = train_test_split_indicies(
            ...
            exclude_val_blocks_dict = None,  # No filtering for training data
            )
        _, val_trials = train_test_split_indicies(
            ...
            seed = self.args['dataset']['seed'],
            bad_trials_dict = None,
            exclude_val_blocks_dict = exclude_val_blocks_dict,  # Filter out OpenWebText/Random blocks
            )
    ...
    def create_optimizer(self):
        ...
        Biases and batch norm parameters should not be decayed
        '''
        # Separate parameters into groups: biases, batch norm params, and other weights
        bias_params = []
        bn_params = []
        other_params = []
        ...
        for name, p in self.model.named_parameters():
            if 'gru.bias' in name or 'out.bias' in name:
                ...
                # Batch normalization parameters (gamma and beta) should not have weight decay
                bn_params.append(p)
            else:
                other_params.append(p)

        param_groups = [
                {'params' : bias_params, 'weight_decay' : 0, 'lr': self.args['lr_max'], 'betas': (self.args['beta0'], self.args['beta1']), 'eps': self.args['epsilon']},
                {'params' : bn_params, 'weight_decay' : 0, 'lr': self.args['lr_max'], 'betas': (self.args['beta0'], self.args['beta1']), 'eps': self.args['epsilon']},
                {'params' : other_params, 'weight_decay' : self.args['weight_decay'], 'lr': self.args['lr_max'], 'betas': (self.args['beta0'], self.args['beta1']), 'eps': self.args['epsilon']}
        ]
        ...
        if len(optim.param_groups) == 2:
            ...
            lr_lambdas = [
                ...
                lambda step: lr_lambda(
                    ...
                    lr_min / lr_max,
                    lr_decay_steps,
                    lr_warmup_steps), # batch norm params
                lambda step: lr_lambda(
                    step,
                    lr_min / lr_max,
                    lr_decay_steps,
            ...
            if (i > 0 and i % self.args['batches_per_val_step'] == 0) or i == ((self.args['num_training_batches'] - 1)):
                ...
                if self.args['log_individual_day_val_PER']:
                    for day in val_metrics['day_PERs'].keys():
                        if val_metrics['day_PERs'][day]['total_seq_length'] > 0:
                            ...
                            self.logger.info(f"{self.args['dataset']['sessions'][day]} val PER: N/A (no validation sequences)")
