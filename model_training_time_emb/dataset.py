...
def get_sinusoidal_time_embedding(day_index, embedding_dim):
    '''
    Generate sinusoidal time embeddings based on day index.
    Similar to positional encoding in transformers.

    Args:
        day_index (int): Day index (0 for first day, 2 for day after 2 days, etc.)
        embedding_dim (int): Dimension of the time embedding

    Returns:
        torch.Tensor: Time embedding of shape (embedding_dim,)
    '''
    position = torch.tensor([day_index], dtype=torch.float32)

    # Create the division term for sine and cosine
    ...
    if embedding_dim > 1:
        embedding[1::2] = torch.cos(position * div_term[:embedding_dim//2 + embedding_dim%2])

    return embedding

def session_to_day_index(session_name, reference_date='2023.08.11'):
    '''
    Convert session name to day index relative to reference date.

    Args:
        session_name (str): Session name in format 't15.YYYY.MM.DD' or 't12.YYYY.MM.DD'
        reference_date (str): Reference date in format 'YYYY.MM.DD' (default: '2023.08.11' = day 0)

    Returns:
        int: Number of days since reference date
    '''
    # Extract date from session name (format: t15.2023.08.11)
    ...
    day_diff = (session_date - ref_date).days
...
class BrainToTextDataset(Dataset):
    ...
    def __init__(
        ...
        time_embedding_dim  (int)       - dimension of time embedding to add (0 to disable time embeddings)
        session_names       ([str])     - list of session names corresponding to day indices (used for time embedding calculation)
         '''
        ...
        # Create mapping from day index to actual day number (relative to reference date)
        self.day_to_day_index = {}
        if self.time_embedding_dim > 0 and self.session_names is not None:
            for day_idx in trial_indicies.keys():
                session_path = trial_indicies[day_idx]['session_path']
                # Extract session name from path
                session_name = [s for s in session_path.split('/') if (s.startswith('t15.20') or s.startswith('t12.20'))][0]
                # Calculate day index relative to reference date (2023.08.11)
                self.day_to_day_index[day_idx] = session_to_day_index(session_name)
                ...
                for t in index[d]:
                    ...
                    try:
                        ...
                        if self.time_embedding_dim > 0:
                            # Get the actual day index (days since reference date)
                            actual_day_index = self.day_to_day_index[d]
                            # Generate time embedding for this day
                            time_emb = get_sinusoidal_time_embedding(actual_day_index, self.time_embedding_dim)
                            # Expand to match number of time steps [num_time_steps, time_embedding_dim]
                            num_time_steps = input_features.shape[0]
                            time_emb_expanded = time_emb.unsqueeze(0).expand(num_time_steps, -1)
                            # Concatenate with input features [num_time_steps, 512 + time_embedding_dim]
                            input_features = torch.cat([input_features, time_emb_expanded], dim=1)
...
def train_test_split_indicies(file_paths, test_percentage = 0.1, seed = -1, bad_trials_dict = None, exclude_val_blocks_dict = None):
    '''
    ...
    Args:
        ...
        exclude_val_blocks_dict (dict): Dictionary of validation blocks to exclude entirely (e.g., OpenWebText/Random corpus blocks).
            Only applies when test_percentage == 1 (validation data). Formatted as:
            {
                'session_name_1': [block_num_1, block_num_2, ...],
                'session_name_2': [block_num_1, block_num_2, ...],
                ...
            }
    '''
        ...
        if os.path.exists(path):
            with h5py.File(path, 'r') as f:
                ...
                for t in range(num_trials):
                    ...
                    # Skip bad trials from bad_trials_dict
                    if (
                    ...
                    # Skip trials from excluded validation blocks (only for validation data)
                    if (
                        test_percentage == 1
                        and exclude_val_blocks_dict is not None
                        and session in exclude_val_blocks_dict
                        and block_num in exclude_val_blocks_dict[session]
                    ):
                        # print(f'Excluding validation trial from block: {session}_{block_num}_{trial_num}')
                        continue
