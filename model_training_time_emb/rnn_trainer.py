...
class BrainToTextDecoder_Trainer:
    ...
    def __init__(self, args):
        ...
        # Add time embedding dimension to input features if time embeddings are enabled
        time_emb_dim = self.args['dataset'].get('time_embedding_dim', 0)
        total_input_dim = self.args['model']['n_input_features'] + time_emb_dim

        self.model = GRUDecoder(
            neural_dim = total_input_dim,
        ...
        # Get time embedding dimension from config (default to 0 if not specified)
        time_embedding_dim = self.args['dataset'].get('time_embedding_dim', 0)
        if time_embedding_dim > 0:
            self.logger.info(f'Using time embeddings with dimension: {time_embedding_dim}')
        ...
        self.train_dataset = BrainToTextDataset(
            ...
            session_names = self.args['dataset']['sessions']
            )
    ...
    def create_optimizer(self):
        ...
        Biases should not be decayed
        '''
        bias_params = [p for name, p in self.model.named_parameters() if 'gru.bias' in name or 'out.bias' in name]
        other_params = [p for name, p in self.model.named_parameters() if 'gru.bias' not in name and 'out.bias' not in name]
